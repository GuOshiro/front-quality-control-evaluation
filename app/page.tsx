...
import {
  generateQualityControlEvaluation,
  retrieveQualityControlEvaluation,
} from "./src/services/QualityControlEvaluation";
...
export default function Home() {
  ...
  const handleOnGenerateRandomData = async () => {
    setIsLoading(true);
    const responseRandom = await generateQualityControlEvaluation();
    const responseControl = await retrieveQualityControlEvaluation(
      responseRandom
    );
    const row = mapRowData(responseRandom, responseControl);
    const updatedRow = [...data, row];
    setData(updatedRow);
    setIsLoading(false);
  };
  ...
  return (
    <main className="flex min-h-screen flex-col gap-10 p-8 lg:p-24 ">
      {isLoading && <Loading />}
      <div>
